...
//declaración de variables y diferentes funciones
...
int main(int argc, char** argv) {           //Función principal en donde se realizan todos los procesos
    ...
    // Se definen tamaño y ubicacion de las texturas
    ...
    //Configuracion los pines del GPIO para obtener el audio por los pines 32 y 33
    system("gpio mode 26 ALT0");
    system("gpio mode 23 ALT0");

    //Se declara el ciclo de reinicio del juego
    ...
    // Terminado el juego se destruyen los recursos
    ...
    SDL_Quit();
    if (sistema==false){system("sudo shutdown now");}// se determina si se presionó enter para apagar la plataforma

    return 0;
}

void Engine(){                      //Motor del juego, realiza todas las acciones del juego llamando a sus diferentes etapas
...
SDL_Texture* load_texture(const char* fname, SDL_Renderer *renderer) {  // Cargar las texturas ya sean imágenes o texto
...
SDL_Texture* cargar_texto(TTF_Font *fuente, const char *texto, SDL_Color color, SDL_Renderer *renderer){    //Carga las frases de texto
...
void RefreshVidas(){            //Imprime las vidas en base a las variables booleanas que las habilitan
if (vidas[0]==true){
    dest_vida.x=690;
...
void RefreshScreen(){           //Actualiza la pantalla de juego
...
void Pprincipal(){                  //Pantalla principal, va tomando el nombre y espera un ENTER, además habilita el menu de ayuda
    while (ciclo){
...
void RefreshBloques(){              //Imprime los bloques que se encuentran "activos"
    int contador=0;
    int despx=1;
    int despy=1;
    int rotBloques=1;
...
int checkColisiones(){              //Detecta las colisiones verticales y horizontales de la bola con el entorno de juego
    int resultado=0;
...
void GameLoop(){                    //ciclo principal de juego
    ...
    while (flag_vidas == false && mainloop && total_vidas !=65 && Boton_presionado==false){
        ...
        while (comienzo == false){
            ...
            SDL_RenderPresent(renderer);                //Coloca las texturas en la pantalla
        }
        RefreshScreen();
        bool loop = true;
...
void RefreshFrame(){                //Refresca la pantalla pero deshabilitando el movimiento de la bola y la plataforma
    SDL_RenderCopy(renderer, fondo, NULL, &dest_rect);
    SDL_RenderCopy(renderer, bola, NULL, &dest_bola);
    SDL_RenderCopy(renderer, marco, NULL, &dest_marco);
    SDL_RenderCopy(renderer, plataforma, NULL, &dest_plat);
...
bool PresionarX(){                  //Lógica para solicitar la tecla x
    SDL_Event teclaX;
    bool salida=false;
...
bool PresionarENTER(){              //Lógica para solicitar la tecla enter
...
bool PressReturn(){              //Lógica que permite regresar al sistema operativo con la tecla r
    SDL_Event TeclaRet;
    bool salida=true;
    while (SDL_PollEvent(&TeclaRet)){
        switch (TeclaRet.type){
            case SDL_KEYDOWN:
            switch (TeclaRet.key.keysym.sym){
                case SDLK_r:
                sistema=true;
                salida = false;
                break;
...
int Pausa(){                        //Lógica para presionar P y se dé la pausa o se reanude el juego
    state = 1;
    SDL_Event pause;
...
void PantallaFinal(){               //Muestra los datos especificados en la pantalla final
    ...
    while (ciclo){
        ...
        ciclo2 = PressReturn();
        if (ciclo2==false){ciclo=false;}
        Boton_presionado = tecladoMatricial();
        if (Boton_presionado){ciclo = false;}
    }
}

bool tecladoMatricial(){            //chequea si se ha presionado asterisco en el teclado matricial para el reinicio del juego
...
void procesador(){                  //Determina la información del procesador
    FILE* file;
...
void performance(){                 //Determina el rendimiento del procesador en la ejecución del juego
    FILE* archivo;
    char porcentaje[6];
    float percent;
...
void instrucciones(){       //muestra la pantalla de ayuda donde indica las instrucciones del juego
    SDL_RenderClear(renderer);
    SDL_RenderCopy(renderer, fondo, NULL, &dest_rect);
    SDL_RenderCopy(renderer, marcop, NULL, &dest_marcop);
